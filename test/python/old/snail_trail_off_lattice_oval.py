...
file_handler = casie.simulation.setup.setup("/home/grogan/Chaste-Test/Casie/Snail-Trail/OffLatticeOval")
...
network = casie.population.vessel.VasculatureGenerator().GenerateOvalNetwork(400, 40, 0.5, 1.0)

# Set up an off lattice tip cell migration rule
migration_rule = casie.simulation.OffLatticeMigrationRule()
...
sprouting_rule = casie.simulation.OffLatticeSproutingRule()
...
manager.Setup()
manager.SetEndTimeAndNumberOfTimeSteps(200.0, 200) # 100 hour simulation, 1 hour dt
