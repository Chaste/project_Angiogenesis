...
class TestOffLatticeAngiogenesisSolver : public AbstractCellBasedTestSuite
{
...
public:
    ...
    void TestSingleVesselGrowth() throw(Exception)
    {
        ...
        OffLatticeAngiogenesisSolver<3> angiogenesis_solver(p_network);
    ...
    void TestMultiSproutWithPde() throw(Exception)
    {
        ...
        p_pde->SetLinearInUTerm(-2.e-6);

        // Set up and run the solver
        boost::shared_ptr<FiniteDifferenceSolver<3> > p_solver = FiniteDifferenceSolver<3>::Create();
        p_solver->SetExtents(p_domain, 2.0);
        p_solver->SetPde(p_pde);

        OutputFileHandler output_file_handler("TestOffLatticeAngiogenesisSolver/MultiSproutPde/", false);
        std::string output_directory = output_file_handler.GetOutputDirectoryFullPath();
    ...
    void TestMultiSproutWithFlow() throw(Exception)
    {
        ...
        OutputFileHandler output_file_handler("TestOffLatticeAngiogenesisSolver/MultiSproutFlow/", false);
        std::string output_directory = output_file_handler.GetOutputDirectoryFullPath();
