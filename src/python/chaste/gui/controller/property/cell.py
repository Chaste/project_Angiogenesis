...
class Controller(casie.controller.base.Controller):
    ...
    def __init__(self, controller_id = 0, label = None, io_data = None):
        ...
        casie.controller.base.Controller.__init__(self, controller_id, "property", "cell", 
                                                  label, io_data)
...
if casie.gui.properties._is_gui_session:
    ...
    class Panel(casie.controller.base.Panel):
        ...
        def __init__(self, parent):
            casie.controller.base.Panel.__init__(self, parent, "property", "cell")
    
        def add_gui_controls(self):
            
            self.consumption_rate_label = wx.StaticText(parent = self, label="Consumption Rate")
            self.consumption_rate_ctrl = wx.TextCtrl(self, value="0.001")
            self.production_rate_label = wx.StaticText(parent = self, label="Production Rate")
            self.production_rate_ctrl = wx.TextCtrl(self, value="0.00")            
        ...
        def on_set(self, event):
            ...
            if self.active_controller is not None:
                props = {"Consumption Rate" :float(self.consumption_rate_ctrl.GetValue()), 
                         "Consumption Rate" : float(self.production_rate_ctrl.GetValue())}
