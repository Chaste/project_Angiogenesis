...
class Controller(casie.controller.base.Controller):
    ...
    def __init__(self, controller_id = 0, label = None, io_data = None):
        ...
        casie.controller.base.Controller.__init__(self, controller_id, "property", "vessel", 
                                                  label, io_data)
...
if casie.gui.properties._is_gui_session:
    ...
    class Panel(casie.controller.base.Panel):
        ...
        def __init__(self, parent):
            casie.controller.base.Panel.__init__(self, parent, "property", "vessel")
    
        def add_gui_controls(self):
            self.consumption_rate_label = wx.StaticText(parent = self, label="Concentration")
            self.consumption_rate_ctrl = wx.TextCtrl(self, value="1.0")
            self.production_rate_label = wx.StaticText(parent = self, label="Permeability")
        ...
        def on_set(self, event):
            ...
            if self.active_controller is not None:
                props = {"Concentration" :float(self.consumption_rate_ctrl.GetValue()), 
                         "Permeability" : float(self.production_rate_ctrl.GetValue())}
