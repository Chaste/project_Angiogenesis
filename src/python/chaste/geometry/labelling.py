...
if casie.gui.properties._have_wx:
    ...
    class BoundaryMarker2dPanel(casie.gui.panels.base.Panel):
        ...
        def on_load_file(self, event = None):
            ...
            self.surface = casie.utility.rwc.read_vtk_surface(self.file_name, True, triangulate=True)
        ...
        def on_save_file(self, event = None):
            ...
            casie.utility.rwc.write_vtk_surface(file_mod_1, self.tool.get_output())
            casie.utility.rwc.write_vtk_surface(file_mod_2, self.tool.open_surface)
...
class BoundaryMarker2d():
    ...
    def update(self):
        ...
        for idx, eachPoint in enumerate(interior_points):
            interior_label.InsertNextTupleValue((float(interior_points[idx]),))  
            
        for idx, eachPoint in enumerate(boundary_points):
